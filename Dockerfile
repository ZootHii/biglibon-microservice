...
if ! docker info >/dev/null 2>&1; then \
  if [ -S /var/run/docker.sock ]; then \
    ...
    echo 'No host Docker socket detected, trying internal dockerd...'; \
    ...
    for i in $(seq 1 180); do \
      ...
      if [ \"$i\" -eq 180 ]; then \
        ...
        echo 'Try: docker run --rm -v /var/run/docker.sock:/var/run/docker.sock docker.io/zoothii/biglibon-microservice:master'; \
